...
impl FileDb {
    ...
    pub fn add(&mut self, file: File) -> usize {
        self.files.push(file);

        self.files.len() - 1
    }

    pub fn get(&self, file_id: usize) -> &File {
        &self.files[file_id]
    }
}
...
impl<'a> codespan_reporting::files::Files<'a> for FileDb {
    ...
    fn name(&self, id: Self::FileId) -> Result<Self::Name, codespan_reporting::files::Error> {
        ...
        return Ok(self.files[id].path.to_string_lossy().to_string());
    }
...
impl File {
    pub fn new(path: PathBuf, content: Vec<u8>, tokens: Option<Vec<Token>>) -> Self {
