...
pub enum TypecheckErrorKind {
    ...
    FunctionCallWrongType(Identifier, Symbol), // Tries to call symbol that is not a function, instead it's a <symbol>
    UnknownIdentifierInExpression(Identifier), // Tries to use an unknown identifier in an expression
    IdentifierIsClassInExpression(Identifier, FileId), // The identifier in an expression is known but a class (which is defined in FileId)
    ...
    ReturnExpressionInVoidFunction(zPAR_TYPE), // The return expression has the given type
    ...
    IdentifierIsNotInstance(Identifier, FileId), // An identifier is used in instance-position (inst.member), but is not suitable. That identifier is defined in FileId
    TypeIsPrimitive(zPAR_TYPE), // A variable of the given type was accessed like an instance (inst.member)
    UnknownMember(Identifier, Identifier, FileId), // Something is trying to access member '2' of class '1', which is defined in '3'
}
